...
- [ ] **PH1-02 — Reject past arrival dates on inquiry submission**
  - [ ] Validate arrival against the current local date in the enhanced submit path before `form.submit()`, including dates entered directly; retain the current date-field relationship, error messaging, and valid Netlify Forms markup and submission path.
  - [ ] Preserve native form constraints when JavaScript is absent or initialization fails. A fixed HTML `min` cannot advance with the date, so it cannot by itself guarantee rejection of every past arrival on later visits.
  - [ ] Resolve the no-JavaScript path with the owner: complete moving-date protection requires a submission-side validation gate that also covers direct posts. Confirm a supported way to reject invalid requests while retaining Netlify Forms collection before adding it; if none is chosen, document the limitation and keep this objective open.
  - [ ] In a browser, test completed past-date and valid future-date inquiries with JavaScript enabled and disabled, inspect native `ValidityState` where applicable, and verify the valid enhanced Netlify submission path without sending a real inquiry. Verify any submission-side gate through its actual request path; do not infer live Netlify delivery from local interception.
